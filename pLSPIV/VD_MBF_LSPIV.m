function [imageLines,startColumn,endColumn,index_vals1,windowsize,velocity,pixel_size,badvals,meanvel,stdvel,Switch_to_mm,numWorkers,fpscheck,full_analysis,index_length,numavgs, skipamt] = VD_MBF_LSPIV( LineScanTable, nImage, mImage, fpscheck, pixel_size, numWorkers, windowsize)
...
if fpscheck == false
    prompt = {"What is the frame rate:", 'Enter pixel size(um):','Number of cores','Full analysis(0=No 1=Yes)'};
    questiontitle = 'Define Parameters';
    dims = [1 35];
    definput = {'120','0.8','8','0'};
    if nImage == 512 && mImage == 1024
        definput = {'30','0.3672','8','0'};
    elseif nImage == (512/2) && mImage == (1024/2)
        definput = {'60','0.3672','8','0'};
...
str = {'capillary', 'artery', 'user', 'user2', 'user3'};
...
elseif speedSetting == 3   % USER SETTING
    ...
    shiftamt      = 1;
elseif speedSetting == 4   % SETTING
    numavgs       = 10;  %up to 100 (or more) for noisy or slow data
    skipamt       = 3;   %if it is 2, it skips every other point.  3 = skips 2/3rds of points, etc.
    shiftamt      = 1;
elseif speedSetting == 5   % SETTING
    numavgs       = 5;  %up to 100 (or more) for noisy or slow data
    skipamt       = 3;   %if it is 2, it skips every other point.  3 = skips 2/3rds of points, etc.
    shiftamt      = 1;
end
